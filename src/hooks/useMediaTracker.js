...
export const MOCK_ITEM_IDS = new Set([
    ...
    if (!user) {
      ...
      if (localData) {
        try {
          ...
          const userOnly = Array.isArray(parsed) ? parsed.filter(item => !MOCK_ITEM_IDS.has(item.id)) : [];
          setMediaItems(processOnePieceItems(userOnly));
        } catch {
          setMediaItems([]);
        }
      } else {
    ...
    const unsubscribe = onValue(mediaRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        Object.keys(val).forEach((key) => {
          if (MOCK_ITEM_IDS.has(key)) {
            mockKeysToRemove.push(key);
          } else {
            itemsArray.push({
              id: key,
              ...val[key],
        ...
        // Clean up legacy mock items from user's Firebase RTDB node
        if (mockKeysToRemove.length > 0) {
          mockKeysToRemove.forEach((mockKey) => {
            remove(ref(db, `users/${user.uid}/mediaTracker/${mockKey}`)).catch(() => {});
          });
        }
