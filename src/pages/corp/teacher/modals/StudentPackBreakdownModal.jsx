                  ...
                  return (
                    <div key={packId} style={{ padding: '12px 14px', borderRadius: '12px', background: 'var(--bg-tertiary)', border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <strong style={{ color: 'var(--text-primary)', fontSize: '0.9rem' }}>{pack ? pack.title : 'Unknown pack'}</strong>
                      ...
                      {agg.hasData ? (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
                          <span>Learned: <strong style={{ color: 'var(--text-primary)' }}>{agg.wordsLearned || 0}</strong></span>
                          ...
                          {agg.atRiskCount > 0 && (
                            <span style={{ color: 'var(--warning)', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                              <AlertTriangle size={13} /> {agg.atRiskCount} need attention
                            </span>
                          )}
                          <span>Last active: {agg.lastActivity ? new Date(agg.lastActivity).toLocaleDateString() : '—'}</span>
                        </div>
                      ) : (
                        <span style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>Not practiced yet.</span>
                      )}
