...
export default function StudentDetailModal({ p }) {
  ...
  // Keep rendering the last-viewed student while the modal plays its exit
  // animation — viewingStudentDetail itself goes null the instant the
  // modal starts closing, and re-rendering with nothing to show would
  // blank the card mid-fade instead of letting it dismiss gracefully.
  const [lastStudent, setLastStudent] = useState(viewingStudentDetail);
  useEffect(() => {
    if (viewingStudentDetail) setLastStudent(viewingStudentDetail);
  }, [viewingStudentDetail]);

  const student = viewingStudentDetail || lastStudent;

  return (
    /* Student Detail Modal — overview + per-pack mastery/retention breakdown */
    <TeacherModal open={!!viewingStudentDetail} onClose={() => setViewingStudentDetail(null)} maxWidth="460px">
      {student && (
        <>
